...
import { buildWordIndex, findWordAtTimeFast, findWordAtCharPositionFast } from "@/lib/word-index"
...
function HighlightedText({
  ...
  for (const match of matches) {
    const globalMatchIdx = allMatches.findIndex(
      (m) => m.segmentId === segmentId && m.startIndex === match.startIndex && m.endIndex === match.endIndex,
    )
...
export function Editor({
  ...
  }, [isPlaying, autoFollowEnabled, currentPlaybackWord, onProgrammaticScroll])
  ...
  const handleSegmentClick = useCallback(
    ...
    [onSelectSegment, scrollToTime, onSeek],
  )
  ...
  return (
    <div className="flex flex-col gap-0 pb-32 min-h-full" ref={containerRef}>
          ...
          >
            ...
            <div className="w-40 flex-shrink-0 flex flex-col gap-2 pt-1 relative">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  setActiveSpeakerDropdown(activeSpeakerDropdown === segment.id ? null : segment.id)
                }}
              >
                <div
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            className={cn(
                              "text-sm",
                              s.id === currentSpeaker?.id ? "font-semibold text-blue-700" : "text-slate-700",
                            )}
                          >
                            {s.name}
                          </span>
                        </div>
                        {s.id === currentSpeaker?.id && <Check size={14} className="text-blue-600" />}
                      </button>
                    ))}
                  </div>
                </div>
      ...
      <div className="h-32 flex items-center justify-center text-slate-300 italic">End of transcription</div>
    </div>
  )
}
