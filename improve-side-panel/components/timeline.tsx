...
import { getSegmentEdgeTimes, type SnapConfig, snapTime, snapToEdge, snapToGrid } from "@/lib/timeline-utils"
...
export function Timeline({
  ...
  useEffect(() => {
    if (!isPlaying || !autoFollowEnabled) return
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (dragState.mode === "left") {
        ...
        const rawEnd = Math.max(dragState.initialStartTime + minDuration, dragState.initialEndTime + deltaTime)
  ...
  }, [dragState, segments, zoomLevel, onUpdateSegment, gridSnap, autoScroll, performEdgeSnap, performSnap])

  const handleSegmentDragStart = (e: React.MouseEvent, segmentId: string, mode: DragMode, segment: Segment) => {
    e.stopPropagation()
    if (!containerRef.current) return
    ...
    <div className="flex flex-col h-full bg-white select-none">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className={cn(
              ...
              gridSnap ? "bg-blue-100 text-blue-700 hover:bg-blue-200" : "text-slate-500 hover:bg-slate-100",
            )}
          ...
          <button
            ...
            className={cn(
              "p-1.5 rounded flex items-center gap-1.5 text-xs font-medium transition-colors",
              dragLineSnap ? "bg-purple-100 text-purple-700 hover:bg-purple-200" : "text-slate-500 hover:bg-slate-100",
            )}
          ...
          <button
            ...
            className={cn(
              "p-1.5 rounded flex items-center gap-1.5 text-xs font-medium transition-colors",
              autoScroll ? "bg-green-100 text-green-700 hover:bg-green-200" : "text-slate-500 hover:bg-slate-100",
            )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex shrink-0 h-8 border-b border-slate-200">
          ...
          >
            <div style={{ width: `${Math.max(100, totalDuration * zoomLevel + 500)}px` }} className="h-full relative">
        ...
        <div ref={scrollContainerRef} className="flex-1 flex overflow-y-auto" onScroll={handleUserScroll}>
          {/* Speakers Column */}
              ...
              >
                ...
                <div className="relative">
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      setActiveMenuSpeakerId(activeMenuSpeakerId === speaker.id ? null : speaker.id)
                      setMergeTargetMode(null)
                    }}
                  >
                          ...
                          <div className="max-h-40 overflow-y-auto">
                            {speakers
                              ...
                              .map((target) => (
                                ...
                                >
                                  <div className="w-2 h-2 rounded-full" style={{ background: target.color }} />
                                  {target.name}
                                </button>
                              ))}
                            {speakers.length <= 1 && (
                              <div className="px-4 py-2 text-xs text-slate-400 italic">No other speakers</div>
                            )}
                          </div>
                          <button
                            type="button"
            ...
            <div style={{ width: `${Math.max(100, totalDuration * zoomLevel + 500)}px` }} className="h-full relative">
              ...
              {gridSnap && zoomLevel > 30 && (
                <div className="absolute top-0 bottom-0 left-0 right-0 pointer-events-none">
                  {Array.from({ length: Math.ceil(totalDuration / gridSnapInterval) + 1 }).map((_, i) => {
                    const time = i * gridSnapInterval
                    const isMainLine = time % 1 === 0 // Full second marks
                    return (
                      <div
                        key={`grid-${i}`}
                        className={cn("absolute top-0 bottom-0 w-px", isMainLine ? "bg-slate-200" : "bg-slate-100")}
                        style={{ left: `${time * zoomLevel}px` }}
                      />
                    )
                  })}
                      ...
                      {speakerSegments.map((seg) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              handleSegmentClick(seg.id)
                              // Calculate proportional time based on click position within segment
                              const rect = e.currentTarget.getBoundingClientRect()
                              const clickX = e.clientX - rect.left
                              const segWidth = rect.width
                              const proportionalTime = start + (clickX / segWidth) * duration
