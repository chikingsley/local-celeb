...
export function Minimap({
	...
	return (
		<div
			...
			className="relative flex w-16 cursor-pointer select-none flex-col border-slate-200 border-l bg-slate-50"
			onClick={handleClick}
		...
		>
			...
			<div className="relative flex-1 px-1.5 py-2">
				{segments.map((segment, _index) => {
					...
					return (
						<div
							className="relative mb-0.5 overflow-hidden rounded-sm"
							key={segment.id}
							style={{
						...
						>
							...
							{hasMatches && (
								<div className="absolute inset-0 flex flex-col justify-center gap-px px-0.5">
									...
									{matchCount > 3 && (
										<div className="h-0.5 rounded-full bg-yellow-400 opacity-50" />
									)}
								</div>
							)}
						</div>
				...
				<div
					...
					aria-valuenow={Math.round(viewportTop)}
					className={cn(
						"pointer-events-auto absolute right-0 left-0 mx-1 rounded border-2 transition-opacity",
						isDragging
			...
			{searchMatches.length > 0 && (
				<div className="absolute bottom-2 left-1/2 -translate-x-1/2 rounded bg-yellow-100 px-1.5 py-0.5 font-medium text-[10px] text-yellow-800">
