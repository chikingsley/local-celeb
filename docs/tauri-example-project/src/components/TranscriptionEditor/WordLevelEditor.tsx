...
import type { Segment, Word } from "../../types/transcription";
	...
	return (
		...
		>
			...
			<div className="flex items-center justify-between mb-3 text-sm text-gray-600">
				...
				<div className="text-xs text-gray-400">Segment ID: {segment.id}</div>
			</div>
			...
			<div className="leading-relaxed text-base">
				{segment.words.map((word, wordIndex) => {
					...
					const isCurrentWord =
						currentWord?.segment.id === segment.id && currentWord?.wordIndex === wordIndex;
...
export const WordLevelEditor: React.FC<WordLevelEditorProps> = ({ className = "" }) => {
	const { transcription, speakers, assignSpeaker, splitSegment } = useTranscriptionStore();
