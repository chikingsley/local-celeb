...
import { exportToCSV, exportToSRT, exportToTXT, exportToVTT } from "../../utils/transcriptionUtils";
...
export const ExportService: React.FC<ExportServiceProps> = ({ className = "" }) => {
	...
	const getWordCount = () =>
		transcription?.segments.reduce((acc, seg) => acc + seg.words.length, 0) || 0;
	...
	return (
		<div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
			<div className="flex items-center justify-between mb-4">
				...
				<Button size="sm" variant="outline" onClick={() => setShowOptions(!showOptions)}>
			...
			{/* Export Button */}
			<Button onClick={exportData} disabled={!transcription} className="w-full">
